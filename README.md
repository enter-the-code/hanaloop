...
| 단계 | 커밋 내용 |
|------|-----------|
| 1 | 타입 생성 (`types.ts`) — 도메인 확장 타입 설계 |
| 2 | 계산식 추가 (`calculations.ts`) — 집계·KPI·포맷 순수 함수 |
| 3 | 폴더 구조 정리 |
| 4 | 시드 데이터·Fake API (`data.ts`, `api.ts`) |
| 5 | KPI 카드·대시보드·그래프·LCA 막대 레이아웃 프레임 |
| 6 | Lucide React 아이콘 추가 + 네비게이션·필터 바 고도화 |
| 7 | Recharts 차트 + 스켈레톤 + 도넛·트렌드·LCA 차트 구현 |
| 8 | 레이아웃 완성 (테이블·NotesPanel·Toast) |
| 9 | 툴팁 완성 — Portal 기반, 도넛 차트 커스텀 툴팁 포함 전 컴포넌트 적용 |
